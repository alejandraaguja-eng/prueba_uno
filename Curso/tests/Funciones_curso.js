...
exports.FJ=class FJ{
    ...
    async valor_texto(selector,tiempo=tie){
        ...
        const value = await this.page.locator(selector).innerText();
        await sleep(tiempo)
        return value
    }
    ...
    async mouse_over(selector,val,tiempo=tie){
        const sel = await this.page.locator(selector)
        await sel.hover();
        await sleep(tiempo)
    }
    ...
    async drag_drop(ori,des,tiempo=tie){
        const origen = await this.page.locator(ori)
        let selectorDestino = des
        if(des.includes("drop-box ui-droppable") && des.includes("[1]")){
            selectorDestino = "(//div[@id='droppable'][contains(.,'Drop Here')])[1]"
        }
        const destinos = await this.page.locator(selectorDestino)
        let destino = destinos.first()
        for(let i=0;i<await destinos.count();i++){
            if(await destinos.nth(i).isVisible()){
                destino = destinos.nth(i)
                break
            }
        }
